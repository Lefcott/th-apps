...
export default class Verify {
  ...
  /**
   * Static method for specific validation actions against shadow dom elements.
   * @example Verify.theShadowElement(sel).isVisibleWithin(selector)
   * @example Verify.theToast.showsUpWithText('content created')
   * @param {string} sel selector of shadow dom element
   * @param {string} selector selector of container of shadow dom
   */
   static theShadowElement = selector => ({
    isVisibleWithin: sel => {
      cy.get(sel).shadow().find(selector).should('be.visible');
    },
  });
